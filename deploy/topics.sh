...
# the old `create ... -p 3 || true` in `make up` silently left every topic at
# 1 partition (Redpanda's auto-create default) while the SQL job reasoned about 3.
set -eu
...
# Flink reads scan.startup.mode = earliest-offset. At the 7-day default a job
# resubmitted after a week silently replays truncated history. 30 days covers any realistic outage.
